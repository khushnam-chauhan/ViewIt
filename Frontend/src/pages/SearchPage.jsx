	...
	const handleSearch = async (e) => {
		...
		try {
			...
			if (error.response.status === 404) {
				toast.error("Nothing here, maybe try searching under the right category");
			} else {
        ...
        <div className='flex justify-center gap-3 mb-4'>
					<button
						className={`py-2 px-4 rounded ${
							activeTab === "movie" ? "bg-pink-800" : "bg-gray-600"
						} hover:bg-pink-700`}
					...
					<button
						className={`py-2 px-4 rounded ${
							activeTab === "tv" ? "bg-pink-800" : "bg-gray-600"
						} hover:bg-pink-700`}
					...
					<button
						className={`py-2 px-4 rounded ${
							activeTab === "person" ? "bg-pink-800" : "bg-gray-600"
						} hover:bg-pink-700`}
                ...
                <form className='flex gap-2 items-stretch mb-8 max-w-2xl mx-auto' onSubmit={handleSearch}>
					<input
						...
						placeholder={" What if, you searched for a ? " + activeTab}
						className='w-full p-2 rounded bg-gray-600 text-white'
					/>
					<button className='bg-pink-700 hover:bg-pink-800 text-white p-2 rounded'>
